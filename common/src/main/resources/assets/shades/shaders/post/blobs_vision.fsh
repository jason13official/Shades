...
/// direct port of 1.20.1's blobs.fsh: a cheap 2-ring directional smooth in each of the 4
/// cardinal directions, averaged together; softer and less "eroded" than antialias_vision's
/// min-based kernel, closer to a soft directional blur
void main() {
    ...
    vec4 l2 = texture(InSampler, texCoord + vec2(-oneTexel.x, 0.0) * 2.0);
    vec4 r1 = texture(InSampler, texCoord + vec2(oneTexel.x, 0.0));
    vec4 r2 = texture(InSampler, texCoord + vec2(oneTexel.x, 0.0) * 2.0);
