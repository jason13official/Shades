...
const vec3 GLOW_COLOR = vec3(1.0, 0.15, 0.85);
...
void main(){
    ...
    // 3x3 Sobel edge magnitude, same kernel xray_vision.fsh uses
    ...
    // cheap bloom: sample luma at a small ring around this pixel and compare against the center -
    // spreads the glow a few pixels past the actual edge instead of a hairline, without a real
    // second blur pass
    ...
    vec3 dimmed = texture(InSampler, texCoord).rgb * 0.15;
